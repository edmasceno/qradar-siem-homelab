...
## 🎯 Visão Geral
Este repositório registra a configuração de um ambiente de monitoramento utilizando o **IBM QRadar Community Edition**. O foco principal foi a resolução de questões críticas de infraestrutura, como a expiração de licenças em snapshots antigos, a recuperação de serviços de banco de dados e a ingestão de logs por meio de PowerShell.
...
A instalação inicial foi realizada a partir de um snapshot limpo do CentOS. O primeiro desafio surgiu ao tentar acessar o sistema remotamente via SSH, quando o Windows 11 recusou a conexão por incompatibilidade com algoritmos legados de autenticação (MACs).

**Ação:** Forçar o uso de `hmac-sha1` para viabilizar o acesso administrativo por meio do terminal.
...
Antes da estabilização completa do SIEM, surgiram problemas na camada de virtualização que impediam a operação adequada da máquina virtual.

* **Erro MachineWrap:** Um erro grave de interface (E_FAIL) no VirtualBox impediu a anexação correta do disco VDI.
* **Solução:** Para restaurar a integridade do boot, foi necessário ajustar as controladoras SATA e IDE nas configurações de armazenamento da VM.
...
Apesar de o serviço de coleta `ecs-ec-ingress` estar em execução, a porta 514 permanecia fechada. A análise dos registros indicou que a licença do QRadar estava expirada.

**Solução:** Foi realizado um "Time Travel" no servidor, ajustando o relógio do sistema para **maio de 2020** e desativando o NTP, permitindo a validação temporária da licença.
...
* **Diagnóstico:** Utilizando o `journalctl`, constatei que o serviço `ariel_query_server` não conseguia iniciar corretamente.
* **Intervenção manual:** Utilizei o `psql` para acessar o banco de dados e investigar tabelas de sistema, além de analisar processos órfãos com o comando `ps aux | grep ariel`, com o objetivo de restaurar o serviço.
...
Desenvolvi um script em PowerShell para converter eventos locais em mensagens Syslog e enviá-las via UDP na porta 514. O tráfego foi validado utilizando `tcpdump` e monitoramento por SSH.
...
Logs inicialmente classificados como "Unknown" foram padronizados. Utilizei o **DSM Editor** para criar o **QID 11750001**, possibilitando o reconhecimento e a classificação adequada do evento `host_Script`.
...
## 📚 Conclusão
Este laboratório demonstrou que a atuação em um SOC vai além do simples monitoramento de painéis. É fundamental possuir domínio sobre infraestrutura, banco de dados e rede para garantir visibilidade e confiabilidade operacional.
